...
private:
    // Velocity limits
    double max_linear_vel = 0.5;    // Maximum forward/backward velocity (m/s)
...
public:
    GoalDriver() : Node("goal_driver"), 
        ...
        RCLCPP_INFO(this->get_logger(), 
                    ...
                    turn_threshold, turn_threshold * 180.0 / M_PI);
    }
...
private:
    ...
    void control_loop() {
        ...
        if (std::abs(yaw_error) > turn_threshold) {
            ...
            cmd.yaw_cmd = std::clamp(1.5 * yaw_error, -max_angular_vel, max_angular_vel);
            
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                               "Turning: yaw_error=%.2f° (%.3f rad)", 
                               yaw_error * 180.0 / M_PI, yaw_error);
        } else {
            // DRIVE FORWARD MODE
            ...
            double linear_vel = 0.5 * distance;
            double angular_vel = 1.0 * yaw_error;  // Still correct heading while driving
            
            cmd.x_cmd = std::clamp(linear_vel, 0.0, max_linear_vel);
            cmd.y_cmd = 0.0;
            cmd.yaw_cmd = std::clamp(angular_vel, -max_angular_vel, max_angular_vel);
            
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                               "Driving: dist=%.2fm, vel=%.2fm/s, time=%.1fs", 
                               distance, cmd.x_cmd, elapsed);
        }
...
// ros2 topic pub /drive_pos rover_messages/msg/AutonDrive "{x_cmd: 0.0, y_cmd: 0.0, stop: false}" --once#include <rclcpp/rclcpp.hpp>
#include <geometry_msgs/msg/pose_stamped.hpp>
#include <tf2_ros/buffer.h>
#include <tf2_ros/transform_listener.h>
#include <tf2_geometry_msgs/tf2_geometry_msgs.hpp>
